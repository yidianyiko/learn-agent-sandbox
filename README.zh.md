...
| **s02** | [自己写一个 hypervisor](s02_write_a_vmm/) | 79 行能跑的 VMM，以及那另外十二万行到底在干什么的**实测答案** | **C** | 是 | ✅ |
| **s03** | [**别启动，恢复。**](s03_snapshot_restore/) | 亲手实现快照与恢复，然后是那两个数字：2659 ms vs 16 ms | **C** + curl | 是 | ✅ |
| **s04** | 控制面与数据面，永不混合 | 一个能起停沙箱的 HTTP 服务 | Go | 是 | 🚧 |
| **s05** | fork 机器，而不是 fork 进程 | 从一个快照分叉出 N 个沙箱并行运行 | Go | 是 | 🚧 |
| **s06** | 里面得有个人 | guest 内的极小静态二进制，可从宿主 `exec` 进去 | Rust | 是 | 🚧 |
