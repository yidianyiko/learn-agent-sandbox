...
**凡是使用现成 API 的章节，必须先把底下的机制亲手实现一遍。**

- 上层调用回答"怎么用"；底层实现回答"为什么是这样"。**只有前者的章节不算完成。**
- 这条不是可选的风格偏好——用户在 s01 和 s03 两次独立提出同一个问题
  （"直接用的现成工具，没有把底层剖析出来"、"一直在上层调用 API，没有深入底层实现"），
  说明默认写法有系统性偏差，必须写成规则而非个案修补。

**已按此原则处理的章节：**

| 章 | 机制（亲手实现） | API（生产工具） |
|---|---|---|
| s02 | `tinyvmm.c` 79 行，六个 ioctl 造出一台 VM | —— |
| s03 | `tinysnap.c` 99 行，GET/SET_REGS + write/read 内存 | Firecracker `/snapshot/create` `/snapshot/load` |
| s04+ | **待定，但必须有** | |

**实现方式**：机制部分用 C（裸 KVM ABI），生产部分用该层真实语言。
两部分放同一章，顺序是先机制后 API——先让读者看见"就这么点东西"，
再看生产实现在此之上加了什么。
